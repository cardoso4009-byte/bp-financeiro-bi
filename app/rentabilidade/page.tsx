...
export default function Rentabilidade(){
 ...
 const scoped=useMemo(()=>operational.filter(e=>scopeKeys.has(e.competence)),[operational,scopeKeys])
 const receita=scoped.filter(e=>e.type==='Receita').reduce((s,e)=>s+Math.abs(e.value),0)
 const opex=scoped.filter(e=>e.type==='Despesa').reduce((s,e)=>s+Math.abs(e.value),0)
 ...
 const previous=scoped.filter(e=>e.competence===previousKey)
 const previousResult=previous.filter(e=>e.type==='Receita').reduce((s,e)=>s+Math.abs(e.value),0)-previous.filter(e=>e.type==='Despesa').reduce((s,e)=>s+Math.abs(e.value),0)
 ...
 return <main className="content" style={{marginLeft:0,width:'100%',maxWidth:1500,margin:'0 auto'}}>
  <header><div><small>CONTROLADORIA FINANCEIRA</small><h1>Rentabilidade Gerencial</h1><p>Receita • OPEX • Resultado • Margem • Conta • Centro de custo • Categoria</p></div><ReportPeriodFilter value={period} onChange={p=>setPeriod(p)} years={[2026]}/></header>
  <div className="cards"><Metric title="Receita" value={brl(receita)} sub={label}/><Metric title="OPEX" value={brl(opex)} sub={label}/><Metric title="Resultado" value={brl(resultado)} sub="Receita − OPEX"/><Metric title="Margem" value={pct(margem)} sub="Resultado / Receita"/></div>
  {period.view==='comparativo'&&<section className="panel wide"><div className="panel-title"><div><h2>Análise Comparativa</h2><span>{label} × {period.month===1?`Dez/${period.year-1}`:`${REPORT_MONTHS[period.month-2]}/${period.year}`}</span></div><span>Rentabilidade</span></div><div className="rows"><div className="row"><span>Resultado atual</span><b>{brl(resultado)}</b></div><div className="row"><span>Resultado anterior</span><b>{brl(previousResult)}</b></div><div className="row"><span>Variação</span><b>{brl(resultado-previousResult)}</b></div></div></section>}
  <section className="panel wide"><div className="panel-title"><div><h2>Mapa de Rentabilidade</h2><span>+ expande • − recolhe • Conta → Centro de custo → Categoria → Mês → Lançamento</span></div><span>{label}</span></div><div className="table-wrap"><table><thead><tr><th style={{textAlign:'left'}}>Dimensão</th><th>Valor</th></tr></thead><tbody>{nodes.map(n=><Node key={n.key} node={n} expanded={expanded} toggle={toggle} onSelect={setSelected} scopeKeys={scopeKeys}/>)}</tbody></table></div></section>
...
function Metric({title,value,sub}:{title:string;value:string;sub:string}){return <div className="card"><span>{title}</span><strong>{value}</strong><small>{sub}</small></div>}
