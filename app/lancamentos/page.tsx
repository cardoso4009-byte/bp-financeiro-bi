...
import { canPostToPeriod, getClosingState } from '@/lib/closing-state'
...
export default function Lancamentos() {
  ...
  const s = summarizeEntries(periodEntries)
  const formLocked = !canPostToPeriod(form.competence)

  function isEntryLocked(entry: FinancialEntry) { return !canPostToPeriod(entry.competence) }
  function addEntry() {
  ...
  function saveEdit() {
    if (editingId === null || formLocked) return
    const value = Number(form.value.replace(',', '.'))
    if (!form.description.trim() || !value) return
    const signed = form.type === 'Receita' ? Math.abs(value) : -Math.abs(value)
  ...
  function duplicateEntry(entry: FinancialEntry) { if (!isEntryLocked(entry)) setEntries(prev => [{ ...entry, id: Date.now(), description: `${entry.description} (cópia)` }, ...prev]) }
  function removeEntry(entry: FinancialEntry) { if (!isEntryLocked(entry)) setEntries(prev => prev.filter(e => e.id !== entry.id)) }
  ...
  return <main className="content" style={{ marginLeft: 0, width: '100%', maxWidth: 1500, margin: '0 auto' }}>
    <header><div><small>CONTROLADORIA FINANCEIRA</small><h1>Base de Lançamentos</h1><p>Entrada financeira • Classificação • Competência • Vencimento • Integração gerencial</p></div><ReportPeriodFilter value={period} onChange={setPeriod} years={[2026]}/></header>
    <div className="cards"><Card title="Receitas" value={s.revenue} sub={`${periodEntries.filter(e => e.type === 'Receita').length} lançamentos • ${label}`} /><Card title="OPEX" value={s.opex} sub="Despesas operacionais" /><Card title="CAPEX" value={s.capex} sub="Investimentos" /><Card title="Em aberto" value={s.open} sub="A pagar / receber" /></div>
    ...
    <div className="grid">
      <section className="panel"><div className="panel-title"><h2>{editingId === null ? 'Novo lançamento' : 'Editar lançamento'}</h2><span>{editingId === null ? 'Entrada manual' : `ID ${editingId}`}</span></div>
        {formLocked && <div className="note" style={{ marginBottom: 12, borderLeft: '4px solid #c33' }}><strong>Competência fechada:</strong> {getClosingState(form.competence).status}. Novos lançamentos e alterações estão bloqueados para este período.</div>}
        <div className="entry-form">
          <Field label="Data"><input type="date" value={form.date} disabled={formLocked} onChange={e=>setForm({...form,date:e.target.value})}/></Field>
          <Field label="Tipo"><select value={form.type} disabled={formLocked} onChange={e=>setForm({...form,type:e.target.value as FinancialEntry['type']})}>{entryTypes.map(x=><option key={x}>{x}</option>)}</select></Field>
          <Field label="Categoria"><input value={form.category} disabled={formLocked} onChange={e=>setForm({...form,category:e.target.value})}/></Field>
          <Field label="Centro de custo"><input value={form.costCenter} disabled={formLocked} onChange={e=>setForm({...form,costCenter:e.target.value})}/></Field>
          <Field label="Descrição"><input value={form.description} disabled={formLocked} onChange={e=>setForm({...form,description:e.target.value})} placeholder="Ex.: fornecedor, cliente..."/></Field>
          <Field label="Competência"><input type="month" value={form.competence} onChange={e=>setForm({...form,competence:e.target.value})}/></Field>
          <Field label="Vencimento"><input type="date" value={form.dueDate} disabled={formLocked} onChange={e=>setForm({...form,dueDate:e.target.value})}/></Field>
          <Field label="Pagamento"><input type="date" value={form.paymentDate} disabled={formLocked} onChange={e=>setForm({...form,paymentDate:e.target.value})}/></Field>
          <Field label="Status"><select value={form.status} disabled={formLocked} onChange={e=>setForm({...form,status:e.target.value as FinancialEntry['status']})}><option>Pago</option><option>Em aberto</option></select></Field>
          <Field label="Valor"><input inputMode="decimal" disabled={formLocked} value={form.value} onChange={e=>setForm({...form,value:e.target.value})} placeholder="0,00"/></Field>
          {editingId === null ? <button className="primary-btn" disabled={formLocked} onClick={addEntry}>+ Adicionar lançamento</button> : <div style={{display:'flex',gap:8}}><button className="primary-btn" disabled={formLocked} onClick={saveEdit}>Salvar alteração</button><button className="delete-btn" onClick={cancelEdit}>Cancelar</button></div>}
        </div>
      </section>
      <section className="panel"><div className="panel-title"><h2>Resumo da base</h2><span>Motor financeiro</span></div><div className="rows"><div className="row"><span>Resultado líquido dos lançamentos</span><b>{brl(s.net)}</b></div><div className="row"><span>Financiamentos</span><b>{brl(s.financing)}</b></div><div className="row"><span>Itens em aberto</span><b>{periodEntries.filter(e=>e.status==='Em aberto').length}</b></div><div className="row"><span>Total de lançamentos</span><b>{periodEntries.length}</b></div></div><div className="note">Fonte única de dados: <strong>{FINANCIAL_STORAGE_KEY}</strong>. A base financeira alimenta a integração contábil e será a origem dos demonstrativos.</div></section>
    </div>
    <section className="panel wide"><div className="panel-title"><h2>Lançamentos financeiros</h2><span>{filtered.length} registros exibidos • {label}</span></div><div className="toolbar"><input placeholder="Pesquisar descrição, categoria ou centro de custo..." value={search} onChange={e=>setSearch(e.target.value)}/><select value={filter} onChange={e=>setFilter(e.target.value)}><option>Todos</option>{entryTypes.map(x=><option key={x}>{x}</option>)}</select></div><div className="table-wrap"><table><thead><tr><td>Data</td><td>Tipo</td><td>Categoria</td><td>Centro de custo</td><td>Descrição</td><td>Competência</td><td>Vencimento</td><td>Status</td><td>Valor</td><td>Ações</td></tr></thead><tbody>{filtered.map(e=><tr key={e.id}><td>{e.date.split('-').reverse().join('/')}</td><td><Tag type={e.type}/></td><td>{e.category}</td><td>{e.costCenter}</td><td>{e.description}</td><td>{e.competence}</td><td>{e.dueDate.split('-').reverse().join('/')}</td><td>{e.status}</td><td className={e.value>=0?'positive':'negative'}>{money(e.value)}</td><td><div style={{display:'flex',gap:6,flexWrap:'wrap'}}><button className="delete-btn" onClick={()=>setViewing(e)}>Visualizar</button><button className="delete-btn" disabled={isEntryLocked(e)} onClick={()=>startEdit(e)}>Editar</button><button className="delete-btn" disabled={isEntryLocked(e)} onClick={()=>duplicateEntry(e)}>Duplicar</button><button className="delete-btn" disabled={isEntryLocked(e)} onClick={()=>removeEntry(e)}>Excluir</button></div></td></tr>)}</tbody></table></div></section>
    ...
    <section className="panel"><div className="panel-title"><h2>Próxima integração</h2><span>Arquitetura</span></div><div className="note"><strong>Base de Lançamentos → classificação contábil → Diário → Razão → Balancete → DRE / BP / DFC / DMPL → Indicadores → Diagnóstico.</strong> A camada de origem já está centralizada; agora o fechamento da competência também controla a possibilidade de alteração dos lançamentos.</div></section>
  </main>
}
