...
export default function Governanca() {
  ...
  return (
    <main className="content" style={{ marginLeft: 0, width: '100%', maxWidth: 1450, margin: '0 auto' }}>
      ...
      <div className="cards"><Metric title="Ações abertas" value={String(openActions.length)} /><Metric title="Ações críticas" value={String(critical.length)} /><Metric title="Ações atrasadas" value={String(overdue.length)} /><Metric title="Vencem em 7 dias" value={String(dueSoon.length)} /><Metric title="Reuniões realizadas" value={String(meetings.filter((m) => m.status === 'Realizada').length)} /></div>
      <section className="panel wide"><div className="panel-title"><div><h2>1. Alertas que exigem gestão</h2><span>{diagnosisSummary(diagnoses)}</span></div><a href="/alertas-gerenciais" style={{ fontWeight: 800 }}>Ver cockpit de alertas →</a></div><div style={{ display: 'grid', gap: 10 }}>{diagnoses.slice(0, 5).map((d) => <div className="note" key={d.key}><strong>{d.title}</strong><p style={{ margin: '4px 0' }}>{d.signal}</p><small>Hipótese: {d.hypothesis}</small></div>)}</div></section>
      <section className="panel wide"><div className="panel-title"><div><h2>2. Plano de ação</h2><span>Transforme diagnóstico em responsável, prazo e evidência</span></div><a href="/plano-acao" style={{ fontWeight: 800 }}>Abrir plano completo →</a></div>{openActions.length === 0 ? <div className="note">Nenhuma ação aberta no momento.</div> : <div style={{ display: 'grid', gap: 10 }}>{openActions.map((a) => <ActionRow key={a.id} item={a} onClose={() => closeAction(a)} />)}</div>}</section>
      <section className="panel wide"><div className="panel-title"><div><h2>3. Reunião de Resultado</h2><span>Registre decisões e evidências da gestão</span></div><button onClick={() => setShowMeeting((v) => !v)}>{showMeeting ? 'Fechar' : '+ Registrar reunião'}</button></div>{showMeeting && <div className="panel" style={{ margin: 0 }}><div className="grid"><Field label="Data" value={form.date} onChange={(v) => setForm({ ...form, date: v })} type="date" /><Field label="Competência" value={form.competence} onChange={(v) => setForm({ ...form, competence: v })} type="month" /><Field label="Título" value={form.title} onChange={(v) => setForm({ ...form, title: v })} /><Field label="Participantes" value={form.participants} onChange={(v) => setForm({ ...form, participants: v })} /></div><TextArea label="Decisões e encaminhamentos" value={form.decisions} onChange={(v) => setForm({ ...form, decisions: v })} /><TextArea label="Evidência / ata / observações" value={form.evidence} onChange={(v) => setForm({ ...form, evidence: v })} /><button onClick={() => saveMeeting()}>Encerrar e registrar reunião</button></div>}<div style={{ display: 'grid', gap: 10, marginTop: 12 }}>{meetings.slice(0, 8).map((m) => <MeetingRow key={m.id} meeting={m} />)}</div></section>
...
function ActionRow({ item, onClose }: { item: ActionPlanItem; onClose: () => void }) { const late = !!item.dueDate && item.dueDate < today(); const border = item.priority === 'Crítica' || late ? '#ef4444' : '#f59e0b'; return <article className="note" style={{ borderLeft: `4px solid ${border}` }}><div className="panel-title"><div><strong>{item.title}</strong><p style={{ margin: '4px 0' }}>{item.area} • {item.responsible || 'Responsável não definido'}</p></div><span>{item.priority}</span></div><small>Prazo: {item.dueDate || 'não definido'} {late ? '• ATRASADA' : ''}</small><p style={{ margin: '6px 0' }}>{item.action}</p><button onClick={onClose}>Marcar como concluída</button></article> }
