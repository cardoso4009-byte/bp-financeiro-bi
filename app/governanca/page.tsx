...
export default function Governanca() {
  ...
  function closeAction(item: ActionPlanItem) {
    const updated = {
      ...
      actualImpact: item.actualImpact || item.expectedImpact,
  ...
  return (
    <main className="content" style={{ marginLeft: 0, width: '100%', maxWidth: 1450, margin: '0 auto' }}>
      <header>
        <div><small>CONTROLADORIA FINANCEIRA</small><h1>Governança Gerencial</h1><p>Alerta → Ação → Reunião → Responsável → Prazo → Evidência → Encerramento</p></div>
      ...
      <section className="panel wide">
        <div className="panel-title"><div><h2>1. Alertas que exigem gestão</h2><span>{diagnosisSummary(diagnoses)}</span></div><a href="/alertas-gerenciais" style={{ fontWeight: 800 }}>Ver cockpit de alertas →</a></div>
      ...
      <section className="panel wide">
        <div className="panel-title"><div><h2>2. Plano de ação</h2><span>Transforme diagnóstico em responsável, prazo e evidência</span></div><a href="/plano-acao" style={{ fontWeight: 800 }}>Abrir plano completo →</a></div>
      ...
      <section className="panel wide">
        <div className="panel-title"><div><h2>3. Reunião de Resultado</h2><span>Registre decisões e evidências da gestão</span></div><button onClick={() => setShowMeeting((v) => !v)}>{showMeeting ? 'Fechar' : '+ Registrar reunião'}</button></div>
...
function ActionRow({ item, onClose }: { item: ActionPlanItem; onClose: () => void }) {
  const late = !!item.dueDate && item.dueDate < today()
  const border = item.priority === 'Crítica' || late ? '#ef4444' : '#f59e0b'
