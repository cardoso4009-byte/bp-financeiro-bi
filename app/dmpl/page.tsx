...
export default function DMPLReconciliationPage() {
  ...
  return <main className="content" style={{ marginLeft: 0, width: '100%', maxWidth: 1400, margin: '0 auto' }}>
    ...
    {period.view === 'comparativo' && <section className="panel wide" style={{ marginBottom: 18 }}><div className="panel-title"><h2>Comparativo da movimentação do PL</h2><span>{previous ? `${REPORT_MONTHS[period.month - 1]} / ${period.year} × ${REPORT_MONTHS[previousPeriod.month - 1]} / ${previousPeriod.year}` : 'Sem período anterior disponível'}</span></div>{previous ? <div className="table-wrap"><table><thead><tr><th>Movimento</th><th style={{ textAlign: 'right' }}>{REPORT_MONTHS[period.month - 1]}/{period.year}</th><th style={{ textAlign: 'right' }}>{REPORT_MONTHS[previousPeriod.month - 1]}/{previousPeriod.year}</th><th style={{ textAlign: 'right' }}>Variação</th></tr></thead><tbody><ComparisonRow title="Lucro Líquido" current={d.lucroLiquido} previous={previous.lucroLiquido} /><ComparisonRow title="PL Calculado" current={d.plCalculado} previous={previous.plCalculado} /><ComparisonRow title="PL Contábil" current={d.plContabil} previous={previous.plContabil} /><ComparisonRow title="Diferença" current={Math.abs(d.diferenca)} previous={Math.abs(previous.diferenca)} /></tbody></table></div> : <div className="note">Não há dados disponíveis para o período imediatamente anterior.</div>}</section>}
...
function ComparisonRow({ title, current, previous }: { title: string; current: number; previous: number }) {
  const delta = current - previous
  const percent = previous === 0 ? null : (delta / Math.abs(previous)) * 100
  return <tr><td><strong>{title}</strong></td><td style={{ textAlign: 'right', fontWeight: 700 }}>{brl(current)}</td><td style={{ textAlign: 'right' }}>{brl(previous)}</td><td style={{ textAlign: 'right', fontWeight: 700 }}>{brl(delta)}{percent === null ? '' : ` (${percent >= 0 ? '+' : ''}${percent.toFixed(1).replace('.', ',')}%)`}</td></tr>
}
