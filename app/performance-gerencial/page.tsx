...
export default function PerformanceGerencial(){
 ...
 const months=Array.from(new Set([...source.entries.map(e=>e.competence),...budget.map(b=>b.competence)])).sort()
...
function sumBudget(items:any[],month:string){return items.filter(x=>x.competence===month).reduce((a,x)=>{const v=Math.abs(Number(x.budget)||0);if(x.type==='Receita')a.revenue+=v;if(x.type==='Despesa')a.opex+=v;if(x.type==='CAPEX')a.capex+=v;if(x.type==='Financiamento')a.financing+=v;return a},{revenue:0,opex:0,capex:0,financing:0})}
