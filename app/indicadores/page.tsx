...
export default function IndicadoresPage() {
  ...
  const periodEntries = useMemo(() => source.entries.filter(e => {
    ...
    const end = `${period.year}-${String(period.month).padStart(2, '0')}`
  ...
  const previousEntries = useMemo(() => {
    ...
    return source.entries.filter(e => (e.competence || e.date.slice(0, 7)) === key)
  }, [source.entries, period])
  const data = useMemo(() => calculateFinancialIndicators(periodEntries), [periodEntries])
  const previousData = useMemo(() => calculateFinancialIndicators(previousEntries), [previousEntries])
  const accountingIndex = Math.max(0, Math.min(period.month - 1, monthlyBalance.length - 1))
  const accounting = useMemo(() => buildAccountingIndicatorCards(accountingIndex), [accountingIndex])
  ...
  const balance = monthlyBalance[accountingIndex] ?? monthlyBalance[monthlyBalance.length - 1]
  const periodLabel = period.view === 'mensal' ? `Mensal • ${balance.month}/${period.year}` : period.view === 'acumulado' ? `Acumulado • Jan–${balance.month}/${period.year}` : `Comparativo • ${balance.month}/${period.year} × mês anterior`
  ...
  return (
    <main className="content" style={{ marginLeft: 0, width: '100%', maxWidth: 1450, margin: '0 auto' }}>
      ...
      <section className="panel wide" style={{ marginBottom: 18 }}>
        <div className="panel-title"><h2>Diagnóstico executivo</h2><span>{periodLabel} • {critical} críticos • {attention} em atenção</span></div>
        <div className="note">
      ...
      {period.view === 'comparativo' && <section className="panel wide" style={{ marginBottom: 18 }}>
        <div className="panel-title"><h2>Comparativo gerencial</h2><span>{balance.month}/{period.year} × mês anterior</span></div>
        <div className="indicator-grid">
          <Metric title="Receita atual" value={brl(data.revenue)} detail={`Anterior: ${brl(previousData.revenue)}`} tone={data.revenue >= previousData.revenue ? 'normal' : 'attention'} />
          <Metric title="EBITDA atual" value={brl(data.ebitda)} detail={`Anterior: ${brl(previousData.ebitda)}`} tone={data.ebitda >= previousData.ebitda ? 'normal' : 'attention'} />
          <Metric title="OPEX atual" value={brl(data.opex)} detail={`Anterior: ${brl(previousData.opex)}`} tone={data.opex <= previousData.opex ? 'normal' : 'attention'} />
          <Metric title="Geração de caixa" value={brl(data.cashGeneration)} detail={`Anterior: ${brl(previousData.cashGeneration)}`} tone={data.cashGeneration >= previousData.cashGeneration ? 'normal' : 'attention'} />
        </div>
      </section>}
      ...
      <section className="panel wide" style={{ marginBottom: 18 }}>
        <div className="panel-title"><div><h2>Indicadores patrimoniais & rentabilidade</h2><span>Data-base: {accounting.snapshot.month}/{period.year}</span></div><span>{accounting.cards.length} indicadores</span></div>
        <div className="indicator-grid">{accounting.cards.map(i => <Metric key={i.key} title={i.label} value={fmt(i.value, i.unit)} detail={i.interpretation} tone={i.tone} />)}</div>
      </section>

      <section className="panel wide" style={{ marginBottom: 18 }}>
        <div className="panel-title"><h2>Indicadores gerenciais de caixa</h2><span>{periodLabel} • Base de lançamentos persistidos</span></div>
        <div className="indicator-grid">
      ...
      <div className="grid">
        <section className="panel"><div className="panel-title"><h2>Capital de Giro & Liquidez</h2><span>Leitura patrimonial</span></div><div className="rows"><Row label="Ativo Circulante" value={balance.ativoCirculante} /><Row label="Passivo Circulante" value={balance.passivoCirculante} /><Row label="Estoques" value={balance.estoques} /><Row label="Capital de Giro Líquido" value={accounting.snapshot.workingCapital} /><Row label="Necessidade de Capital de Giro" value={accounting.snapshot.ncg} /><Row label="Ciclo Financeiro" value={accounting.snapshot.cashConversionCycle} currency={false} suffix=" dias" /></div></section>
...
function DiagnosisCard({ diagnosis }: { diagnosis: { severity: DiagnosisSeverity; title: string; signal: string; hypothesis: string; action: string } }) { const label = diagnosis.severity === 'critical' ? 'CRÍTICO' : diagnosis.severity === 'attention' ? 'ATENÇÃO' : 'POSITIVO'; const tone = diagnosis.severity === 'positive' ? 'normal' : diagnosis.severity; return <div className={`row indicator-${diagnosis.severity}`} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 16, alignItems: 'start', padding: '14px 0' }}><span><b>{diagnosis.title}</b><small style={{ display: 'block', opacity: .85, marginTop: 4 }}><strong>Sinal:</strong> {diagnosis.signal}</small><small style={{ display: 'block', opacity: .75, marginTop: 3 }}><strong>Hipótese gerencial:</strong> {diagnosis.hypothesis}</small><small style={{ display: 'block', opacity: .9, marginTop: 5 }}><strong>Ação recomendada:</strong> {diagnosis.action}</small></span><b className={`tone-${tone}`}>{label}</b></div> }
