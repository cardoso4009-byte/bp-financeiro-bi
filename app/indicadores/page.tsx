...
import {buildFinancialDiagnosis,diagnosisSummary,type DiagnosisSeverity} from '@/lib/financial-diagnosis'
...
export default function IndicadoresPage(){
 ...
 return <main className="content" style={{marginLeft:0,width:'100%',maxWidth:1450,margin:'0 auto'}}>
  ...
  <section className="panel wide" style={{marginBottom:18}}><div className="panel-title"><div><h2>Diagnóstico de causas e ações</h2><span>Regras gerenciais aplicadas aos indicadores integrados</span></div><span>{diagnoses.length} sinal(is)</span></div><div className="rows">{diagnoses.map(d=><DiagnosisCard key={d.key} diagnosis={d}/>)}</div></section>
...
function DiagnosisCard({diagnosis}:{diagnosis:{severity:DiagnosisSeverity;title:string;signal:string;hypothesis:string;action:string}}){const label=diagnosis.severity==='critical'?'CRÍTICO':diagnosis.severity==='attention'?'ATENÇÃO':'POSITIVO';return <div className={`row indicator-${diagnosis.severity}`} style={{display:'grid',gridTemplateColumns:'1fr auto',gap:16,alignItems:'start',padding:'14px 0'}}><span><b>{diagnosis.title}</b><small style={{display:'block',opacity:.85,marginTop:4}}><strong>Sinal:</strong> {diagnosis.signal}</small><small style={{display:'block',opacity:.75,marginTop:3}><strong>Hipótese gerencial:</strong> {diagnosis.hypothesis}</small><small style={{display:'block',opacity:.9,marginTop:5}><strong>Ação recomendada:</strong> {diagnosis.action}</small></span><b className={`tone-${diagnosis.severity==='positive'?'normal':diagnosis.severity}`}>{label}</b></div>}
