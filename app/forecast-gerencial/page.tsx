...
export default function ForecastGerencial(){
 ...
 return <main className="content" style={{marginLeft:0,width:'100%',maxWidth:1450,margin:'0 auto'}}><header><div><small>CONTROLADORIA FINANCEIRA</small><h1>Forecast Gerencial</h1><p>Realizado + projeção futura = visão antecipada do resultado</p></div><ReportPeriodFilter value={period} onChange={setPeriod} years={[2026]}/></header>
 <section className="panel wide"><div className="filters" style={{marginTop:0}}><label>Data-base <input type="date" value={asOf} onChange={e=>setAsOf(e.target.value)}/></label><span className="note">A data-base separa realizado e forecast. O período selecionado define a visão do relatório.</span></div></section>
