...
function Executive(){
  ...
  return <>
    ...
    <Panel title="Evolução mensal — Receita Líquida"><div className="monthly-chart">{monthlyData.map(m=><div className="month-col" key={m.month}><div className="month-value">{brl(m.receitaLiquida)}</div><div className="month-bar"><i style={{height:`${m.receitaLiquida/maxRevenue*100}%`}}/></div><span>{m.month}</span></div>)}</div></Panel>
    ...
    <Panel title="Leitura gerencial"><div className="note">O modelo demonstra lucro líquido de R$ 120 mil, geração operacional de caixa de R$ 120 mil e caixa final de R$ 100 mil. A nova área de Integração Financeira permite conferir como lançamentos pagos e em competência formam DRE e DFC.</div></Panel>
  </>
}
