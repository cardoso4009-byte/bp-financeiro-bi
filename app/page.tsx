...
const modules = [['Gestão Financeira','/gestao'],['Capital de Giro','/capital-giro'],['Fluxo de Caixa','/fluxo-caixa'],['Lançamentos','/lancamentos'],['Integração Financeira','/integracao'],['Contabilidade','/contabil']] as const
...
export default function Home() {
  ...
  const [period, setPeriod] = useState<Period>({ start: 0, end: 11 })
  const item = nav.find(x => x[1] === page)?.[0]
  const selectedMonths = monthlyData.slice(period.start, period.end + 1)
  const periodLabel = period.start === period.end ? months[period.start] : `${months[period.start]}–${months[period.end]}`
  return <main className="shell">
    <aside className="side"><div className="brand"><b>BP</b><div><strong>BP Financeiro</strong><span>Controladoria & BI</span></div></div><div className="company"><small>EMPRESA DEMONSTRATIVA</small><strong>Grupo Exemplo</strong><span>2026 • Modelo integrado</span></div><nav>{nav.map(([label,id]) => <button key={id} className={page===id?'active':''} onClick={()=>setPage(id)}>{label}</button>)}<div className="nav-divider">MÓDULOS GERENCIAIS</div>{modules.map(([label,url])=><a className="nav-link" key={url} href={url}>{label}</a>)}</nav><footer>V1.5 • Projeto Consultoria Financeira</footer></aside>
    <section className="content"><header><div><small>CONTROLADORIA FINANCEIRA</small><h1>{item}</h1><p>Demonstrações integradas • Regime de competência</p></div><div className="period-controls"><label>Período de análise</label><div><select value={period.start} onChange={e=>setPeriod(p=>({start:Number(e.target.value),end:Math.max(Number(e.target.value),p.end)}))}>{months.map((m,i)=><option key={m} value={i}>Início: {m}/2026</option>)}</select><select value={period.end} onChange={e=>setPeriod(p=>({start:Math.min(p.start,Number(e.target.value)),end:Number(e.target.value)}))}>{months.map((m,i)=><option key={m} value={i}>Fim: {m}/2026</option>)}</select></div><span>{periodLabel}/2026</span></div></header>
      {page==='exec' && <Executive selectedMonths={selectedMonths}/>} {page==='dre' && <DRE/>}{page==='bp' && <BP/>}{page==='dfc' && <DFC/>}{page==='dmpl' && <DMPL/>}{page==='ind' && <Indicators/>}
    </section>
  </main>
}

function Executive({selectedMonths}:{selectedMonths: typeof monthlyData}){
  const maxRevenue = Math.max(...monthlyData.map(m=>m.receitaLiquida))
  const receita = selectedMonths.reduce((s,m)=>s+m.receitaLiquida,0)
  return <>
    ...
    <Panel title="Evolução mensal — Receita Líquida"><div className="monthly-chart">{selectedMonths.map(m=><div className="month-col" key={m.month}><div className="month-value">{brl(m.receitaLiquida)}</div><div className="month-bar"><i style={{height:`${m.receitaLiquida/maxRevenue*100}%`}}/></div><span>{m.month}</span></div>)}</div></Panel>
    ...
    <Panel title="Leitura gerencial"><div className="note">Período selecionado: <strong>{selectedMonths[0]?.month} a {selectedMonths[selectedMonths.length-1]?.month}/2026</strong>. Os demonstrativos estruturais permanecem integrados ao modelo contábil.</div></Panel>
  </>
}
