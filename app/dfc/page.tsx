...
export default function DFCPage(){
 ...
 useEffect(()=>{setV2Entries(readV2BrowserStore().entries)},[])
 ...
 const v2Report=useMemo(()=>buildV2Dfc(v2Base,selectedV2Period),[v2Base,selectedV2Period])
 const v2=v2Report.selectedPeriod
 ...
 <section className="panel wide"><div className="panel-title"><h2>DFC V2 — caixa realizado</h2><span>Dados persistidos • competência × liquidação</span></div>
 {v2?<><div className="cards"><Card title="Operacional V2" value={v2.operacional}/><Card title="Investimentos V2" value={v2.investimento}/><Card title="Financeiro V2" value={v2.financeiro}/><Card title="Variação de Caixa" value={v2.variacaoCaixa}/></div><div className="grid"><section className="panel"><div className="panel-title"><h2>Fluxo por natureza</h2><span>{selectedV2Period}</span></div><div className="rows"><div><span>Operacional</span><strong>{brl(v2.operacional)}</strong></div><div><span>Investimento</span><strong>{brl(v2.investimento)}</strong></div><div><span>Financeiro</span><strong>{brl(v2.financeiro)}</strong></div><div><span>Transferências</span><strong>{brl(v2.transferencia)}</strong></div><div><span>Outros</span><strong>{brl(v2.outros)}</strong></div><div><span>= Variação líquida</span><strong>{brl(v2.variacaoCaixa)}</strong></div></div></section><section className="panel"><div className="panel-title"><h2>Governança do caixa</h2><span>Rastreabilidade</span></div><div className="rows"><div><span>Lançamentos liquidados</span><strong>{v2Report.cashSettledEntries}</strong></div><div><span>Lançamentos não liquidados</span><strong>{v2Report.unsettledEntries}</strong></div><div><span>Caixa sem data de liquidação</span><strong>{v2Report.cashBasisWithoutSettlement}</strong></div><div><span>Entradas no período</span><strong>{v2.entries}</strong></div></div></section></div></>:<div className="note">Ainda não há dados V2 liquidados para {selectedV2Period}. Importe um CSV aprovado em Importação para alimentar esta visão. A DFC V1 abaixo permanece disponível.</div>}
 <div className="note">DFC V2 considera caixa somente quando existe settlementDate. Não inventa saldo inicial/final nem ajusta diferenças artificialmente.</div></section>
 ...
 <div className="note">Fonte V1: Financial Core → DRE/BP → Motor DFC • Fonte V2: Base Financeira → liquidação. A V1 permanece inalterada.</div></section></main>
}
function Card({title,value}:{title:string;value:number}){return <div className="card"><span>{title}</span><strong>{brl(value)}</strong><small>motor DFC</small></div>}
