...
import { buildManagementAlerts, buildForecastManagementAlerts, createManagementAction, DEFAULT_MANAGEMENT_THRESHOLDS, type V2ManagementAction, type V2ManagementAlert, type ManagementCauseType, type ManagementAlertLevel } from '@/lib/v2-management'
...
export default function ControladoriaGerencialV2(){
 ...
 const current=selected?[...alerts,...forecastAlerts].find(a=>a.id===selected):undefined
 ...
 const saveCause=()=>{if(!current)return;const updated=updateManagementAlertCause(current,causeType,causeNote);const next=[...alerts,...forecastAlerts].map(a=>a.id===updated.id?updated:a);setSavedAlerts(next);writeV2ManagementAlerts(next);setCauseNote(updated.causeNote??'')}
 ...
 return <main className="page bp-view">
  ...
  <section className="panel" style={{marginBottom:14}}><div className="panel-title"><h2>Alertas de Forecast</h2><span>Orçamento × projeção futura</span></div>{forecastFiltered.length?forecastFiltered.map(a=><div className="alert" key={a.id} onClick={()=>{setSelected(a.id);setCauseType(a.causeType??'nao_classificada');setCauseNote(a.causeNote??'')}}><strong>{a.label}</strong><span className={'badge '+levelClass(a.level)}>{levelLabel[a.level]}</span><small>{a.period} • Forecast {brl(a.budget+a.variance)} • Desvio {brl(a.variance)} {a.variancePercent!==undefined?'• '+(a.variancePercent*100).toFixed(1).replace('.',',')+'%':''}</small></div>):<p>Nenhum desvio projetado acima do limite para as competências futuras.</p>}<div className="note">Esses alertas são calculados sobre o Forecast futuro. A causa continua sendo registrada pelo gestor; o sistema não infere a explicação.</div></section>
