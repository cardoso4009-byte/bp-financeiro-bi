...
export default function Opex(){
 ...
 useEffect(()=>{setEntries(readFinancialSource().entries);const store=readV2BrowserStore();setV2Entries(store.entries);setV2Centers(readV2CostCenters())},[])
 ...
 const v2Report=useMemo(()=>v2Entries.length?buildV2CostCenterReport(buildV2FinancialBase(v2Entries),v2Centers,selectedV2Period):null,[v2Entries,v2Centers,selectedV2Period])
 const v2OpexTotal=v2Report?.rows.reduce((s,row)=>s+row.opex,0)??0
 const v2CapexTotal=v2Report?.rows.reduce((s,row)=>s+row.capex,0)??0
 return <main className="content" style={{marginLeft:0,width:'100%',maxWidth:1450,margin:'0 auto'}}>
  ...
  <section className="panel wide"><div className="panel-title"><div><h2>OPEX V2 por centro de resultado</h2><span>Base Financeira V2 • competência {selectedV2Period}</span></div><span>{v2Report ? v2Report.assignedEntries + '/' + v2Report.totalEntries + ' com centro' : 'Aguardando dados'}</span></div>{!v2Report?<div className="note">Ainda não há lançamentos V2 persistidos para esta competência. A visão V1 continua disponível.</div>:<><div className="cards"><div className="card"><span>OPEX V2</span><strong>{brl(Math.abs(v2OpexTotal))}</strong><small>Classes OPEX</small></div><div className="card"><span>CAPEX V2</span><strong>{brl(Math.abs(v2CapexTotal))}</strong><small>Separado do resultado</small></div><div className="card"><span>Sem centro</span><strong>{v2Report.unassignedEntries}</strong><small>Requer classificação</small></div></div><div className="table-wrap" style={{marginTop:16}}><table><thead><tr><td>Centro</td><td>Lançamentos</td><td>OPEX</td><td>% do OPEX</td><td>CAPEX</td></tr></thead><tbody>{v2Report.rows.map(row=><tr key={row.costCenterId??'sem'}><td><strong>{row.code}</strong><small style={{display:'block'}}>{row.name}</small></td><td>{row.entries}</td><td className="amount">{brl(Math.abs(row.opex))}</td><td className="amount">{v2OpexTotal?((Math.abs(row.opex)/Math.abs(v2OpexTotal))*100).toFixed(1).replace('.',',')+'%' : '—'}</td><td className="amount">{brl(Math.abs(row.capex))}</td></tr>)}</tbody></table></div></>}</section>
