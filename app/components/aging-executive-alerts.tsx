...
export default function AgingExecutiveAlerts(){
 ...
 const dueSoon=items.filter(i=>{const d=daysBetween('2026-12-31',i.dueDate);return d>=0&&d<=30})
 ...
 return createPortal(<section className="panel wide aging-executive-alerts" style={{margin:'16px 0'}}>
  ...
  <div className="grid">
   ...
   <Alert icon="◷" title="Próximos 30 dias" value={brl(dueSoonReceivable)} detail={`${dueSoon.filter(i=>i.direction==='Receber').length} recebimento(s) • ${brl(dueSoonPayable)} em obrigações`} color="#b54708"/>
  ...
  <div className="note" style={{marginTop:12}}><strong>Leitura executiva:</strong> {overdueReceivable>0?`há ${brl(overdueReceivable)} em recebíveis vencidos, reduzindo a previsibilidade do caixa.`:'não há recebíveis vencidos na data-base.'} {dueSoonPayable>0?` Existem ${brl(dueSoonPayable)} em obrigações nos próximos 30 dias.`:''} <a href="/contas-receber-pagar" style={{fontWeight:700,marginLeft:6}}>Abrir Aging →</a></div>
 </section>,target)
}
