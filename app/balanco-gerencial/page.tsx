...
function Group({title,value,children,defaultOpen=true}:GroupProps){
 ...
 return <>
  <tr className="group-row" onClick={()=>setOpen(!open)}>
   <td><button className="expand-btn" aria-label={open?'Recolher':'Expandir'}>{open?'−':'+'}</button><strong>{title}</strong></td>
   <td className="amount"><strong>{brl(value)}</strong></td>
  </tr>
  {open&&children}
 </>
}

function Item({label,value,indent=1}:{label:string;value:number;indent?:number}){
 return <tr className="item-row"><td style={{paddingLeft: `${28+indent*20}px`}}>{label}</td><td className="amount">{brl(value)}</td></tr>
}

function Check({ok,label,value}:{ok:boolean;label:string;value:string}){
 return <div className={`check ${ok?'ok':'warning'}`}><span>{ok?'✓':'!'}</span><div><strong>{label}</strong><small>{value}</small></div></div>
}
...
export default function BalancoGerencial(){
 ...
 const rows=useMemo(()=>[
  ['Liquidez Corrente',m.ativoCirculante/m.passivoCirculante],
  ['Participação do PL',m.pl/m.ativoTotal],
  ['Endividamento',m.passivoTotal/m.ativoTotal],
  ['Imobilização do PL',m.ativoNaoCirculante/m.pl],
  ['Caixa / Ativo',m.caixa/m.ativoTotal]
 ],[m])
 return <main className="bp-page">
  <div className="bp-container">
   <header className="bp-header">
    <div><small>CONTROLADORIA FINANCEIRA</small><h1>Balanço Patrimonial Gerencial</h1><p>Posição patrimonial • análise mensal e comparativa</p></div>
    <select value={base} onChange={e=>setBase(Number(e.target.value))}>{months.map((x,i)=><option value={i} key={x}>Data-base: {x}/2026</option>)}</select>
   </header>
   ...
   {mode==='mensal'&&<>
    ...
    <section className="bp-card">
     <div className="bp-card-title"><div><small>ESTRUTURA PATRIMONIAL</small><h2>Ativo, Passivo e Patrimônio Líquido</h2></div><span>{months[base]}/2026</span></div>
     <div className="table-wrap"><table className="bp-table"><thead><tr><th>Valores em R$</th><th>Saldo</th></tr></thead><tbody>
      <Group title="ATIVO" value={m.ativoTotal}>
       <Group title="Ativo Circulante" value={m.ativoCirculante}>
        <Item label="Caixa" value={m.caixa}/><Item label="Contas a Receber" value={m.contasReceber}/><Item label="Estoques" value={m.estoques}/><Item label="Outros Ativos" value={m.outrosAtivos}/>
       </Group>
       <Group title="Ativo Não Circulante" value={m.ativoNaoCirculante}>
        <Item label="Imobilizado" value={m.imobilizado}/>
       </Group>
      </Group>
      <Group title="PASSIVO" value={m.passivoTotal}>
       <Group title="Passivo Circulante" value={m.passivoCirculante}>
        <Item label="Fornecedores" value={m.fornecedores}/><Item label="Obrigações" value={m.obrigacoes}/><Item label="Outros Passivos" value={m.outrosPassivos}/>
       </Group>
       <Group title="Passivo Não Circulante" value={m.passivoNaoCirculante}>
        <Item label="Dívidas de Longo Prazo" value={m.dividasLongoPrazo}/>
       </Group>
      </Group>
    ...
    <div className="bp-checks">
     ...
     <Check ok={capitalGiro>=0} label={capitalGiro>=0?'Capital de Giro Líquido':'Capital de Giro Líquido Negativo'} value={`${brl(capitalGiro)} • ${capitalGiro>=0?'folga financeira de curto prazo':'necessidade de financiamento de curto prazo'} • variação vs. mês anterior: ${pct(capitalGiroVariation)}`}/>
    </div>
   </>}
