...
assert.equal(audit.settlement.unsettledEntries,3)
assert.equal(audit.settlement.cashBasisWithoutSettlement,2)
assert.equal(audit.duplicateExternalIds,1)
assert.equal(audit.dreVsDfc.dreResult,900)
assert.equal(audit.dreVsDfc.cashVariation,600)
assert.equal(audit.dreVsDfc.bridgeDifference,-300)
assert.equal(audit.periodVariation.dreResultVariation,900)
