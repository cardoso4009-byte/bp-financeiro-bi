...
if (report.passivo !== 0) throw new Error(`BP Gate: passivo esperado 0, recebido ${report.passivo}`)
if (report.patrimonioLiquido !== 1000) throw new Error(`BP Gate: PL esperado 1000, recebido ${report.patrimonioLiquido}`)
if (report.passivoMaisPatrimonioLiquido !== 1000) throw new Error('BP Gate: P + PL incorreto')
if (report.diferencaPatrimonial !== -300) throw new Error(`BP Gate: diferença esperada -300, recebido ${report.diferencaPatrimonial}`)
