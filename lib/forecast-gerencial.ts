...
import type { BudgetPlan } from './budget-plan-data'
...
const add=(a:Bucket,b:Bucket):Bucket=>({revenue:a.revenue+b.revenue,opex:a.opex+b.opex,capex:a.capex+b.capex,financing:a.financing+b.financing})
const monthLabels=['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez']
const budgetLabel=(competence:string)=>monthLabels[Number(competence.slice(5,7))-1]
function entriesByMonth(entries:FinancialEntry[], competence:string):Bucket{return entries.filter(e=>e.competence===competence).reduce((a,e)=>{const v=Math.abs(e.value);if(e.type==='Receita')a.revenue+=v;if(e.type==='Despesa')a.opex+=v;if(e.type==='CAPEX')a.capex+=v;if(e.type==='Financiamento')a.financing+=v;return a},zero())}
function budgetByMonth(items:BudgetPlan[], competence:string):Bucket{const row=items.find(x=>x.month===budgetLabel(competence));return row?{revenue:row.revenue,opex:row.opex,capex:row.capex,financing:0}:zero()}
export function buildAnnualForecast(entries:FinancialEntry[], budgetItems:BudgetPlan[], year:number, asOf:string, actions:ActionPlanItem[]=[]):AnnualForecast{
 ...
 const rows:ForecastRow[]=months.map(m=>{const isActual=m<=cutoff;const b=isActual?entriesByMonth(entries,m):budgetByMonth(budgetItems,m);const net=b.revenue-b.opex-b.capex+b.financing;const source:'Realizado'|'Forecast'=isActual?'Realizado':'Forecast';return {...b,competence:m,isActual,source,net}})
