...
function budgetByMonth(items:BudgetPlan[], competence:string){const row=items.find(x=>x.month===budgetLabel(competence));return row?{revenue:row.revenue,opex:row.opex,capex:row.capex,financing:0}:zero()}
export function buildAnnualForecast(entries:FinancialEntry[], budgetItems:BudgetPlan[], year:number, asOf:string, actions:ActionPlanItem[]=[]):AnnualForecast{
 ...
 const future=futureMonths.reduce((a,m)=>{const b=budgetByMonth(budgetItems,m);const hasBudget=Object.values(b).some(v=>Number(v)>0);const projected=hasBudget?b:{revenue:avg('revenue'),opex:avg('opex'),capex:avg('capex'),financing:avg('financing')};return add(a,projected)},zero())
