...
function seedBudget(): V2BudgetEntry[] {
  return initialBudget.flatMap((row, index) => {
...
export function readV2BudgetEntries(): V2BudgetEntry[] {
  if (typeof window === 'undefined') return seedBudget()
  try {
    const raw = window.localStorage.getItem(V2_BUDGET_STORAGE_KEY)
    if (!raw) return seedBudget()
    const parsed = JSON.parse(raw)
    return Array.isArray(parsed) ? parsed as V2BudgetEntry[] : seedBudget()
  } catch {
    return seedBudget()
  }
}

export function writeV2BudgetEntries(entries: V2BudgetEntry[]): void {
  if (typeof window === 'undefined') return
  window.localStorage.setItem(V2_BUDGET_STORAGE_KEY, JSON.stringify(entries))
}
