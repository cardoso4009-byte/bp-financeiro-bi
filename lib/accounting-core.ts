...
// Regra estrutural: cada lançamento possui exatamente um débito e um crédito
// pelo mesmo valor. O balanceamento do Diário é condição para alimentar o Razão.
export function journalTotals(entries: JournalEntry[]) {
  const debit = entries.reduce((total, entry) => total + entry.amount, 0)
  const credit = entries.reduce((total, entry) => total + entry.amount, 0)
