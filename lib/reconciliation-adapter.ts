...
/** Single source of truth for cross-statement checks. */
export function reconcileFinancialStatements(data: FinancialData): ReconciliationResult {
  ...
  const caixa = data.caixaFinal - (data.caixaInicial + data.caixaOperacional + data.investimentos + data.financiamentos)
