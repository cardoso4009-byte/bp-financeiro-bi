...
function contribution(entry: V2BaseRecord): number {
  const positiveForDre = entry.movementClass === 'receita'
  return positiveForDre
...
export function buildV2DreFromEntries(entries: V2BaseRecord[], period?: string): V2DreReport {
  const periods = [...new Set(entries.map((entry) => entry.period))].sort().map((item) => calculatePeriod(entries, item))
  return { periods, selectedPeriod: period ? periods.find((item) => item.period === period) : undefined }
}
