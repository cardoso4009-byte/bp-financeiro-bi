...
export default function FinancialDrilldown({entries,title='Detalhamento financeiro'}:Props){
 ...
 const centers=unique(entries.map(e=>e.costCenter||'Sem centro de custo'))
 const categories=(center:string)=>unique(entries.filter(e=>(e.costCenter||'Sem centro de custo')===center).map(e=>e.category||'Sem categoria'))
 const months=(center:string,category:string)=>unique(entries.filter(e=>(e.costCenter||'Sem centro de custo')===center&&(e.category||'Sem categoria')===category).map(e=>e.competence||e.date?.slice(0,7)||'Sem competência')).sort()
